...
aws lambda update-function-configuration --function-name auth-handler --environment Variables={ORIGIN=http://localhost:3000} --output text
aws lambda wait function-updated --function-name auth-handler
VERSION=$(aws lambda publish-version --function-name auth-handler --description 'dev' --output text --query Version)
